...
def main(target: int, path_attacked_models_folder: pathlib.Path) -> None:
    ...
    for m, PATH in enumerate(sorted(attack_folder.rglob("*.pth"),reverse=True), 1):
        ...
        # Model
        net_attacked = ResNet50(num_classes=NUM_CLASSES)
        ...
        attacked_site = attack_config['glayer_num'], attack_config['block_num'], attack_config['conv_num']
        ...
        dir_fooling_images += f"glayer{attacked_site[0]}_"
        dir_fooling_images += f"block{attacked_site[1]}_"
        ...
        for q, (img, lb) in enumerate(
            ...
            for j in loop:
                with torch.autocast(
                    ...
                    total_loss, channel_loss = loss(input_img, base_img, val_range, attack_config)
        ...
        fdir = dirMetrics
        fdir += f'/metrics_glayer{attacked_site[0]}_block{attacked_site[1]}_conv{attacked_site[2]}_'
...
if __name__ == "__main__":
    ...
    # path_net_valid = os.path.join(work_dir, "valid_model_checkpoint/resnet50_valid.pth")
    # valid_tuned = True

    path_net_valid = os.path.join(work_dir, "resnet50-0676ba61.pth")  #! original weights pytorch
    valid_tuned = False
    ...
    net_valid = ResNet50(num_classes=NUM_CLASSES)
