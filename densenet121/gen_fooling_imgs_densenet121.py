...
def main(target: int, path_attacked_models_folder: pathlib.Path) -> None:
    ...
    for m, PATH in enumerate(sorted(attack_folder.rglob("*.pth"), reverse=True), 1):
        ...
        # Model
        net_attacked = DenseNet121(num_classes=NUM_CLASSES)
        ...
        attacked_site = (
            attack_config["dense_num"],
            attack_config["dlayer_num"],
            attack_config["conv_num"],
            attack_config["_Transition_num"],
            attack_config["attack_firstRELU"],
            attack_config["attack_lastRELU"],
        )
        ...
        dir_fooling_images += f"dnum{attacked_site[0]}_"
        dir_fooling_images += f"dlayer{attacked_site[1]}_"
        dir_fooling_images += f"conv{attacked_site[2]}_"
        dir_fooling_images += f"trnum{attacked_site[3]}_"
        dir_fooling_images += f"firstRELU{attacked_site[4]}_"
        dir_fooling_images += f"lastRELU{attacked_site[5]}_"
        ...
        def loss(
            input_img: Tensor, base_img: Tensor, val_range: float, attack_config: dict
        ) -> Tuple[Tensor, Tensor]:
            conv_result = net_attacked._forward_generate(input_img, attack_config)
        ...
        for q, (img, lb) in enumerate(
            ...
            for j in loop:
                with torch.autocast(
                    ...
                    total_loss, channel_loss = loss(
                        input_img, base_img, val_range, attack_config
                    )
            ...
            print(
                "Confidence:", confidence, "| Exploit successful:", exploit_successful
            )
        ...
        fdir = dirMetrics
        fdir += f"/metrics_dnum{attacked_site[0]}_dlayer{attacked_site[1]}_conv{attacked_site[2]}_tr{attacked_site[3]}_firstRelu{attacked_site[4]}_lastRelu{attacked_site[5]}_"

        fdir_confs = dirMetrics

        if isinstance(faulted_channel, int):
            fdir += f"channel{faulted_channel}.pkl"
            fdir_confs += f"/confsVals_layer{faulted_channel}.pkl"
        else:
            several = "several"
            fdir += f"channel{several}.pkl"
            fdir_confs += f"/confsVals_layer{several}.pkl"
...
def _load_state_dict(model, weights) -> None:
...
if __name__ == "__main__":
    ...
    # path_net_valid = os.path.join(work_dir, "valid_model_checkpoint/densenet121_valid.pth")
    # valid_tuned = True

    path_net_valid = os.path.join(
        work_dir, "densenet121-a639ec97.pth"
    )  #! original weights pytorch
    valid_tuned = False

    print("path_net_valid:", path_net_valid)
    ...
    net_valid = DenseNet121(num_classes=NUM_CLASSES)
    ...
    if valid_tuned:
        ...
        _load_state_dict(model=net_valid, weights=state_dict)
    net_valid.eval()
