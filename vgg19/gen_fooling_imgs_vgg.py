...
def main(vgg_name: str, target: int, path_attacked_models_folder: pathlib.Path) -> None:
    ...
    for m, PATH in enumerate(sorted(attack_folder.rglob("*.pth")), 1):
        ...
        def loss(
            ...
            global loss_value_ini
            if loss_value_ini is None:
                loss_value_ini = int(channel_loss.item())
                print ("loss_value_ini:", loss_value_ini)
            ...
            # fn_loss = alpha*(ssim_loss/2) + (1-alpha)*(channel_loss/loss_value_ini)
            fn_loss = ssim_loss + channel_loss/loss_value_ini

            return fn_loss, channel_loss/loss_value_ini
